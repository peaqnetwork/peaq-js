...
export class Did extends Base {
  ...
  public async create(
    ...
    statusCallback: (response: ISubmittableResult) => void = () => ({})
    ...
    try {
      ...
      const keyPair = this._metadata?.pair || getKeyPair(seed);
      ...
      const unsubscribe = await attributeExtrinsic.send(statusCallback);
